...
/** Budowa URL CSV z gsheet_url (+ opcjonalnie nazwa karty) – wersja "gviz" działająca z opublikowanym arkuszem */
function buildCsvUrl(sheetUrl: string, sheetName?: string | null) {
  const m = sheetUrl.match(/spreadsheets\/d\/([a-zA-Z0-9-_]+)/)
  const id = m?.[1]
  if (!id) return null

  // Jeśli mamy nazwę karty (nie numeryczne gid) – użyjemy ?tqx=out:csv&sheet=Nazwa
  ...
  // W przeciwnym razie spróbuj z gid z URL-a
  ...
  // Fallback: domyślna karta jako CSV
...
export default async function EditTurniejPage({
  ...
  // ---- PODGLĄD Z ARKUSZA ----
  // teraz: pobieramy całe CSV, ale i tak bierzemy tylko 1 kolumnę (kolumna nazwisk) i od określonego wiersza
  ...
  if (t.gsheet_url && t.kolumna_nazwisk && t.pierwszy_wiersz_z_nazwiskiem) {
    try {
      ...
      if (!resp.ok) {
        throw new Error(`Nie udało się pobrać CSV (${resp.status})`)
      }
      const text = await resp.text()

      const rows = parseCsv(text)
      const colIdx = colLetterToIndex(t.kolumna_nazwisk)
      if (colIdx == null) throw new Error('Nieprawidłowa kolumna (np. B)')

      // wewnętrzny indeks 0-based, a numer wiersza w arkuszu 1-based
      preview = rows
        .map((row, idx) => ({
          row: idx + 1,
    ...
    <main className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-8">
      <div className="w-full max-w-6xl rounded-2xl bg-slate-800/95 border border-slate-700 shadow-[0_14px_40px_rgba(0,0,0,0.8)] p-6 space-y-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Zarządzanie danymi turnieju i uczestnikami
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-6 border-t border-slate-600">
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs">
                            Użyj importu lub dodaj ręcznie
                          </p>
                        </div>
                      </td>
                    </tr>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-sky-100">
              Podgląd z arkusza (kolumna {t.kolumna_nazwisk})
            </h3>
            <p className="text-sm text-slate-400">
              ...
              {typeof t.pierwszy_wiersz_z_nazwiskiem === 'number' &&
                ` • Od wiersza: ${t.pierwszy_wiersz_z_nazwiskiem}`}
            </p>
                  ...
                  {preview.map((item) => {
                    ...
                    if (rowData) {
                      ...
                      if (parts.length >= 2) {
                        ...
                        imie = parts.join(' ')
                        if (imie && nazwisko)
                          norm = normTxt(`${imie} ${nazwisko}`)
                      }
                    }

                    const exists = norm
                      ? existsMap.get(norm) === true
                      : false
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs">
                            Uzupełnij link, kolumnę i pierwszy wiersz
                          </p>
                        </div>
                      </td>
                    </tr>
