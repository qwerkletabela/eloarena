...
export async function POST(request: Request) {
  ...
  // Pobierz konfigurację ELO (rozszerzoną)
  ...
  // Oblicz nowe ELO
  const noweElo = calculateEloChanges(graczeDoObliczen, fullConfig);
  ...
  const partiaData: any = {
    ...
    duzy_punkt_gracz_id: wybraniGracze.find(g => g.duzy_punkt)?.id
  };
  ...
  wybraniGracze.forEach((g, index) => {
    ...
    partiaData[`k_factor${index+1}`] = elo?.k_factor;
  });
