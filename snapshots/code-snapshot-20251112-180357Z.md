# Code snapshot (2025-11-12T18:03:56Z)

## Git

```
commit 8b47c36a0850908a0969174732649d3a2b1edf1e
Author:     MichaÅ‚ <qwerkle.tabela@gmail.com>
AuthorDate: Wed Nov 12 19:03:42 2025 +0100
Commit:     MichaÅ‚ <qwerkle.tabela@gmail.com>
CommitDate: Wed Nov 12 19:03:42 2025 +0100

    fix(MapPicker): type Google Maps click event (google.maps.MapMouseEvent) and add @types/google.maps
```
...
```
total 320
drwxr-xr-x 9 runner runner   4096 Nov 12 18:03 .
drwxr-xr-x 3 runner runner   4096 Nov 12 18:03 ..
drwxr-xr-x 7 runner runner   4096 Nov 12 18:03 .git
drwxr-xr-x 3 runner runner   4096 Nov 12 18:03 .github
-rw-r--r-- 1 runner runner    480 Nov 12 18:03 .gitignore
-rw-r--r-- 1 runner runner   1450 Nov 12 18:03 README.md
drwxr-xr-x 8 runner runner   4096 Nov 12 18:03 app
-rw-r--r-- 1 runner runner    403 Nov 12 18:03 code-snapshot.md
drwxr-xr-x 2 runner runner   4096 Nov 12 18:03 components
-rw-r--r-- 1 runner runner   3364 Nov 12 18:03 db.txt
-rw-r--r-- 1 runner runner    465 Nov 12 18:03 eslint.config.mjs
drwxr-xr-x 3 runner runner   4096 Nov 12 18:03 lib
-rw-r--r-- 1 runner runner    156 Nov 12 18:03 next.config.ts
-rw-r--r-- 1 runner runner 249796 Nov 12 18:03 package-lock.json
-rw-r--r-- 1 runner runner    746 Nov 12 18:03 package.json
-rw-r--r-- 1 runner runner     82 Nov 12 18:03 postcss.config.js
drwxr-xr-x 2 runner runner   4096 Nov 12 18:03 public
drwxr-xr-x 2 runner runner   4096 Nov 12 18:03 snapshots
-rw-r--r-- 1 runner runner    246 Nov 12 18:03 tailwind.config.js
-rw-r--r-- 1 runner runner    666 Nov 12 18:03 tsconfig.json
...
export default async function NewTurniejPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto max-w-2xl p-6 space-y-6">
      ...
      {ok && <div className="rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700">Turniej zostaÅ‚ utworzony.</div>}
      {err && (
        <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
          {{
      ...
      <form action="/turniej/create" method="post" className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-medium">SzerokoÅ›Ä‡ geogr. (lat)</label>
            <input type="number" step="any" name="lat" className="mt-1 w-full rounded border px-3 py-2" placeholder="np. 52.2297" />
          </div>
          <div>
            <label className="block text-sm font-medium">DÅ‚ugoÅ›Ä‡ geogr. (lng)</label>
...
export default async function TurniejListPage() {
  // Uwaga: Å¼adnego pobierania usera i Å¼adnych redirectÃ³w!
  const supabase = await createSupabaseServer()
  ...
  if (error) {
    // pokaÅ¼ Å‚agodny komunikat zamiast przekierowania do logowania
    return (
      <main className="mx-auto max-w-4xl p-6">
        <h1 className="text-2xl font-semibold mb-4">Turnieje</h1>
        <p className="text-sm text-red-700">Nie udaÅ‚o siÄ™ pobraÄ‡ listy turniejÃ³w.</p>
      </main>
    )
  }
  ...
  return (
    <main className="mx-auto max-w-4xl p-6 space-y-4">
      ...
      <ul className="divide-y rounded border">
        {rows?.map(r => (
          <li key={r.id} className="px-4 py-3">
            ...
            <div className="text-xs opacity-70">
              {r.data_turnieju ?? 'â€”'}
              {r.godzina_turnieju ? ` â€¢ ${r.godzina_turnieju}` : ''}
              {r.limit_graczy ? ` â€¢ limit: ${r.limit_graczy}` : ''}
            </div>
            {r.gsheet_url && (
              <div className="mt-2">
        ...
        {!rows?.length && <li className="px-4 py-6 text-sm opacity-60">Brak turniejÃ³w.</li>}
      </ul>
    </main>
  )
}
