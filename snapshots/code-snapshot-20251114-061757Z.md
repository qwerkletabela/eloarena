# Code snapshot (2025-11-14T06:17:55Z)

## Git

```
commit f7bcebd140fe218e2b55a243ab05bbe7daa90a43
Author:     MichaÅ‚ <qwerkle.tabela@gmail.com>
AuthorDate: Fri Nov 14 07:17:44 2025 +0100
Commit:     MichaÅ‚ <qwerkle.tabela@gmail.com>
CommitDate: Fri Nov 14 07:17:44 2025 +0100

    feat(admin/turniej): import graczy z Google Sheets (CSV) do gracz + wyniki; guzik 'Wczytaj z arkusza'
```
...
```
total 324
drwxr-xr-x 9 runner runner   4096 Nov 14 06:17 .
drwxr-xr-x 3 runner runner   4096 Nov 14 06:17 ..
drwxr-xr-x 7 runner runner   4096 Nov 14 06:17 .git
drwxr-xr-x 3 runner runner   4096 Nov 14 06:17 .github
-rw-r--r-- 1 runner runner    480 Nov 14 06:17 .gitignore
-rw-r--r-- 1 runner runner   1450 Nov 14 06:17 README.md
drwxr-xr-x 8 runner runner   4096 Nov 14 06:17 app
-rw-r--r-- 1 runner runner    405 Nov 14 06:17 code-snapshot.md
drwxr-xr-x 2 runner runner   4096 Nov 14 06:17 components
-rw-r--r-- 1 runner runner   7652 Nov 14 06:17 db.txt
-rw-r--r-- 1 runner runner    465 Nov 14 06:17 eslint.config.mjs
drwxr-xr-x 3 runner runner   4096 Nov 14 06:17 lib
-rw-r--r-- 1 runner runner    156 Nov 14 06:17 next.config.ts
-rw-r--r-- 1 runner runner 249796 Nov 14 06:17 package-lock.json
-rw-r--r-- 1 runner runner    746 Nov 14 06:17 package.json
-rw-r--r-- 1 runner runner     82 Nov 14 06:17 postcss.config.js
drwxr-xr-x 2 runner runner   4096 Nov 14 06:17 public
drwxr-xr-x 2 runner runner   4096 Nov 14 06:17 snapshots
-rw-r--r-- 1 runner runner    246 Nov 14 06:17 tailwind.config.js
-rw-r--r-- 1 runner runner    666 Nov 14 06:17 tsconfig.json
...
export default async function EditTurniejPage({ params, searchParams }: { params: Params, searchParams: SearchParams }) {
  ...
  // (opcjonalnie) lista uczestnikÃ³w z widoku v_wyniki â€“ podglÄ…d poniÅ¼ej
    ...
    <main className="mx-auto max-w-2xl p-6 space-y-6">
      ...
      {err && (
        <AutoHide>
          <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
            ...
            {err === 'missing_sheet_cfg' && 'UzupeÅ‚nij link arkusza, kolumnÄ™ i pierwszy wiersz.'}
            {err === 'bad_sheet_url' && 'NieprawidÅ‚owy link do arkusza Google.'}
            {err === 'fetch_failed' && 'Nie udaÅ‚o siÄ™ pobraÄ‡ CSV z arkusza (sprawdÅº udostÄ™pnienie).'}
            {err === 'col_invalid' && 'NieprawidÅ‚owa kolumna (wpisz literÄ™, np. B).'}
            {err === 'empty_after_parse' && 'Po parsowaniu arkusza nie znaleziono Å¼adnych nazwisk.'}
            {err === 'gracz_upsert_failed' && 'BÅ‚Ä…d podczas zapisu graczy.'}
            {!['no_tournament','missing_sheet_cfg','bad_sheet_url','fetch_failed','col_invalid','empty_after_parse','gracz_upsert_failed'].includes(err) && 'Nie udaÅ‚o siÄ™ zapisaÄ‡.'}
          </div>
        </AutoHide>
      )}
      ...
      <section className="mt-8 space-y-3">
        ...
        <p className="text-xs text-slate-600">
          Å¹rÃ³dÅ‚o: {t.gsheet_url ? <a className="underline" href={t.gsheet_url} target="_blank" rel="noreferrer">Google Sheet</a> : 'â€”'}
          {t.arkusz_nazwa ? <> â€¢ karta: <strong>{t.arkusz_nazwa}</strong></> : null}
      ...
      {/* --- PodglÄ…d uczestnikÃ³w (jeÅ›li jest widok v_wyniki) --- */}
      <section className="space-y-2">
        <h3 className="text-base font-semibold">Lista uczestnikÃ³w</h3>
            ...
            <tbody>
              {uczestnicy?.map((r, i) => (
                <tr key={r.id} className="border-b last:border-0">
                  ...
                  <td className="px-3 py-2">{r.club ?? 'â€”'}</td>
              ...
              {!uczestnicy?.length && (
                <tr>
                  <td className="px-3 py-6 text-sm opacity-60" colSpan={7}>
                    Brak uczestnikÃ³w. UÅ¼yj â€Wczytaj z arkuszaâ€.
                  </td>
                </tr>
              )}
            </tbody>
...
type RowT = {
  ...
  gsheet_url: string | null
  arkusz_nazwa: string | null
  kolumna_nazwisk: string | null // np. "B" (peÅ‚ne imiÄ™+nazwisko w jednej kolumnie)
...
function buildCsvUrl(sheetUrl: string, sheetName?: string | null) {
  ...
  // JeÅ›li podano nazwÄ™ karty (nie liczbowy gid) â€“ uÅ¼yj gviz CSV
  ...
  // JeÅ¼eli w URL juÅ¼ jest gid â€“ uÅ¼yj go
...
/** Prosty parser CSV: radzi sobie z cudzysÅ‚owami i separatorami , lub ; */
function parseCsv(text: string): string[][] {
  // heurystyka separatora: jeÅ›li w pierwszej linii wiÄ™cej ; niÅ¼ , to ;
...
/** Rozbija peÅ‚ne imiÄ™+nazwisko na {imie, nazwisko}. Ostatnie sÅ‚owo â†’ nazwisko. */
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // 2) zbuduj URL CSV i pobierz treÅ›Ä‡
  ...
  // 5) upsert graczy po unikalnym fullname_norm (generowana â€“ ale conflict dziaÅ‚a)
  //    Uwaga: Supabase upsert przyjmie conflict po nazwie constraintu albo kolumny; tu uÅ¼yjemy kolumny
  ...
  // 6) pobierz ID wszystkich graczy odpowiadajÄ…cych picked (by uniknÄ…Ä‡ case/ogonki problemÃ³w)
  //    Prosto: po imie+nazwisko ktÃ³re wÅ‚aÅ›nie upsertowaliÅ›my:
  const names = grPayload.map(x => `${x.imie} ${x.nazwisko}`)
  // (jeÅ›li listy duÅ¼e, moÅ¼na dzieliÄ‡ na paczki)
  const { data: allMatched } = await supabase
    .from('gracz')
    .select('id, imie, nazwisko')
    .in('fullname_norm', names.map(n => n.toLowerCase().normalize('NFKD'))) // orientacyjnie
  // Uproszczenie: uÅ¼yjemy id z `inserted` oraz zignorujemy brakujÄ…ce
